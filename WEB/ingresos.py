...
def entrada():
    if request.method == 'POST':
        ...
        sheet.append_row([timestamp, nombre, correo, "Entrada"], value_input_option='RAW')
...
def salida():
    ...
    for registro in registros:
        if len(registro) >= 4:  # Asegurarse de que hay al menos 4 columnas
            timestamp_str = registro.get('Marca temporal', '').strip()  # Marca de tiempo en la columna A
            nombre = registro.get('Nombre completo', '').strip()  # Nombre completo en la columna B
            accion = registro.get('Acción', '').strip().lower()  # Acción en la columna D
            ...
            try:
                ...
                logging.error(f"Formato de tiempo inválido: {timestamp_str}, error: {e}")
                continue
